...
echo "🔍 Checando containers ativos..."
...
if [ -z "$POSTGRES_STATUS" ]; then
  echo "⚠️  Postgres não encontrado. Subindo containers..."
  docker compose up -d
  exit 0
fi

if [ "$POSTGRES_STATUS" != "healthy" ]; then
  echo "⚠️  Postgres não está saudável (status: $POSTGRES_STATUS). Reiniciando..."
  docker restart autoescola_postgres
fi

if [ "$PGADMIN_STATUS" != "healthy" ] && [ "$PGADMIN_STATUS" != "starting" ]; then
  echo "⚠️  PgAdmin não está saudável (status: $PGADMIN_STATUS). Reiniciando..."
  docker restart autoescola_pgadmin
fi

echo "✅ Containers estão rodando normalmente!"
