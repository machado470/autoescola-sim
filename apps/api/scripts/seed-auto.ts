...
async function run() {
  ...
  // 1) se existir delegate "user", usa diretamente
  ...
  // 2) descobrir schema não-sistêmico
  const schemas: any[] = await prisma.$queryRawUnsafe(`
    SELECT nspname AS schema
    FROM pg_namespace
    WHERE nspname NOT IN ('pg_catalog','information_schema')
    ORDER BY (
      SELECT COUNT(*) FROM pg_class c WHERE c.relnamespace = pg_namespace.oid
    ) DESC, nspname ASC
  `);
  ...
  for (const s of schemas.map(x => x.schema)) {
    ...
    `);
    if (cand?.[0]?.table_name) {
      found = { schema: s, table: cand[0].table_name };
      break;
    }
  }
  ...
  const passCol = cols.some(c => c.column_name === "password")
    ? "password"
    : cols.some(c => c.column_name === "hash")
    ? "hash"
    : null;
...
run()
  .catch(e => {
    console.error("❌ seed:", e?.message || e);
    process.exit(1);
  })
